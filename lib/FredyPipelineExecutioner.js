...
class FredyPipelineExecutioner {
   ...
   * and the optional move-in date range (moveInEarliest/moveInLatest).
   *
   * The date range only ever excludes a listing when it HAS a concrete
   * availability date that falls outside the range; listings without a date
   * (availableFrom == null) are always kept, matching the inclusive semantics
   * of the numeric spec filters. ISO 'YYYY-MM-DD' strings compare
   * lexicographically, so plain string comparison is chronologically correct.
   *
  ...
  _filterBySpecs(newListings) {
    const { minRooms, minSize, maxPrice, moveInEarliest, moveInLatest } = this._jobSpecFilter || {};

    // If no specs are set, return all listings
    if (!minRooms && !minSize && !maxPrice && !moveInEarliest && !moveInLatest) {
      return newListings;
    }
    ...
    const keptListings = newListings.filter((listing) => {
      const filterOut =
        ...
        (listing.availableFrom != null &&
          ((moveInEarliest && listing.availableFrom < moveInEarliest) ||
            (moveInLatest && listing.availableFrom > moveInLatest)));
