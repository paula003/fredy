...
import { normalizeAvailabilityDate } from '../utils/normalize-availability-date.js';
...
async function pushDetails(listing) {
  ...
  listing.availableFrom = extractAvailableFrom(detailBody) ?? listing.availableFrom ?? null;

  return listing;
}

/**
 * Find the move-in / availability date in an expose's attribute lists. ImmoScout
 * exposes it as a "Bezugsfrei ab:" (sometimes "Verfügbar ab:" / "frei ab")
 * attribute; the value is a German 'DD.MM.YYYY' date (or empty when the seller
 * left it blank, in which case null is returned).
 *
 * @param {any} detailBody Parsed expose JSON.
 * @returns {string|null} ISO 'YYYY-MM-DD' or null.
 */
function extractAvailableFrom(detailBody) {
  const attributes = (detailBody.sections || [])
    .filter((s) => s.type === 'ATTRIBUTE_LIST')
    .flatMap((s) => s.attributes || []);
  const match = attributes.find((attr) => attr?.label && /bezugsfrei ab|verfügbar ab|frei ab/i.test(attr.label));
  return normalizeAvailabilityDate(match?.text);
}
