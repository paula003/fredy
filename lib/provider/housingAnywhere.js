...
import { firstAvailabilityDate } from '../utils/normalize-availability-date.js';
...
async function getListings(url) {
  ...
  return (body.hits || []).map((hit) => {
    ...
    return {
      ...
      // `firstDate` is the consistent "available from" signal; the others are
      // fallbacks and may carry sentinel/booking-window values that the shared
      // helper skips (it picks the first candidate yielding a real date).
      availableFrom: firstAvailabilityDate([hit.firstDate, hit.bookableDateFromStart, hit.relevantAvailabilityDateTS]),
    };
  });
}
