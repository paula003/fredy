...
function normalize(o) {
  ...
  // The per-card `.printonly` block holds a "Verfügbar: <date>" line; take the
  // text after that label and let the shared parser pull out the actual date.
  const availabilityText = typeof o.availableFrom === 'string' ? o.availableFrom.split(/Verfügbar:/i)[1] : null;
  return {
    id,
    link,
    title: o.title || '',
    ...
    availableFrom: normalizeAvailabilityDate(availabilityText),
  };
}
...
const config = {
  ...
  crawlFields: {
    ...
    availableFrom: '.printonly |removeNewline |trim',
  },
